...
function prod_img($images) {
    ...
    foreach ($images as $img) {
        // New shape: validate the path token directly (regex needs to see _low_/_high_).
        if (!empty($img['path'])) {
            $p = (string)$img['path'];
            if (preg_match('/(^|[\/_.\-])high([\/_.\-]|$)/i', $p)) continue;
            if (!preg_match('/(^|[\/_.\-])low([\/_.\-]|$)/i', $p)) continue;
...
function enrich_products(array $products, bool $listing_mode = false): array {
    ...
    // Set produto_imagem to a placeholder — filled in below after optional filtering.
    ...
    if ($listing_mode) {
        $products = array_values(array_filter($products, fn($p) => !empty($p['produto_imagem'])));
    }
